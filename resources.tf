...
resource "aws_instance" "main" {
  ...
  # The templatefile function starts with the file you want to reference and then a map of values you want to replace in the file.
...
resource "terraform_data" "webapp" {
  ...
  triggers_replace = [
  length(aws_instance.main.*.id), join(",", aws_instance.main.*.id)]
